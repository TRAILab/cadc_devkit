...
class PubTracker(object):
  def __init__(self, hungarian=False, max_age=0, conf_thres=0.0):
  ...
  def step_centertrack(self, results, time_lag):
    if len(results) == 0:
      self.tracks = []
      return [] # TODO: return last frame objects instead (umatched case)
    else:
      temp = []
      for det in results:
        ...
        if det['detection_name'] not in NUSCENES_TRACKING_NAMES:
          continue
        if det['detection_score'] < self.conf_thres:
          continue

        det['ct'] = np.array(det['translation'][:2])
        det['tracking'] = np.array(det['velocity'][:2]) * -1 * time_lag
        det['tracking'] = np.zeros_like(det['velocity'][:2]) # Hack to disable tracking # TODO: vel filter instead
